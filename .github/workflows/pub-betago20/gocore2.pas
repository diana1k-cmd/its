...
begin
case status of
   Suspended: writeln2(7,'Suspended '+str2(moveCount));
   Running:  writeln2(7,'Running '+str2(moveCount));
   Paused2:   writeln2(7,'Paused  '+str2(moveCount));
   Halted:   writeln2(7,'Halted  '+str2(moveCount));
   else ;
   end; // case

   writeln2(3,'WhStonesAvail/Capt->'+str2(goResWh.stonesAvail)+':'+str2(captCount[1]));
   writeln2(4,'BlStonesAvail/Capt->'+str2(goResBl.stonesAvail)+':'+str2(captCount[2]));
   lc:=boardArea(goBoard,whiteStone);
   clusterInfo(lc2,x,y,isSafe,whiteStone);
   writeln2(5,'WhiteCluster->'+str2(lc));
   lc:=boardArea(goBoard,blackStone);
   clusterInfo(lc2,x,y,isSafe,blackStone);
   writeln2(6,'BlackCluster->'+str2(lc));
   writeln2(8,'PurpleStones Avail/Capt->'+str2(goResPurple.stonesAvail)+':'+str2(captCount[3]));
   lc:=boardArea(goBoard,purpleStone);
   clusterInfo(lc2,x,y,isSafe,purpleStone);
   writeln2(9,'PurpleCluster->'+str2(lc));
end;
...
do for j:=1 to MaxY
   do begin
    ...
    on E: EInOutError do
      writeln('RMA(1010) File handling error occurred. Details: ', E.ClassName, '/', E.Message);
  end;
